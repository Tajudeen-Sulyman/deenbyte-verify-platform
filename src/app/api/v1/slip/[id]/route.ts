...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Digital NIN Slip - ${esc(ref)}</title><style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .card::before{content:'';position:absolute;top:0;left:0;right:0;bottom:0;background-image:url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23059669' fill-opacity='0.05'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E");opacity:0.3}
    .header{text-align:center;margin-bottom:15px;position:relative;z-index:1}
    .header h1{color:#059669;font-size:18px;font-weight:700;margin-bottom:2px}
    .header p{color:#047857;font-size:11px;font-weight:600}
    ...
    .details{flex:1}
    .qr{width:80px;height:80px;background:#fff;padding:4px;border-radius:4px}
    ...
    .nin-display{background:#fff;padding:12px;border-radius:4px;margin-top:12px;text-align:center}
    ...
    .issue-date{position:absolute;top:20px;right:20px;text-align:right;z-index:1}
    .issue-date .label{font-size:9px;color:#666}
    .issue-date .value{font-size:11px;font-weight:600;color:#000}
    ...
    .ref{background:#f5f5f5;padding:8px 15px;font-size:10px;color:#666;border-bottom:1px solid #ddd}
    ...
    <div class="card">
      ...
      <div class="content">
        ...
        <div class="details">
          ...
          <div class="qr">
            <img src="https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=${encodeURIComponent(JSON.stringify({ref, nin: d.nin, name: d.first_name + ' ' + d.last_name}))}" alt="QR Code" />
          </div>
        </div>
      </div>
      <div class="nin-display">
        <div class="nin-label">National Identification Number (NIN)</div>
        <div class="nin-number">${esc(d.nin || '').replace(/(\\d{4})(\\d{4})(\\d{4})(\\d{4})/, '$1 $2 $3 $4')}</div>
      </div>
    </div>
    <div class="ref">Ref: ${esc(ref)} | Service: ${esc(svc || 'NIN Verification')} | Status: ${esc(status)}</div>
