...
export async function GET(req: NextRequest) {
  ...
  const { data: row } = await admin.from('cac_applications').select('*').eq('reference', reference).maybeSingle();
  ...
  if (v?.status === true && v?.data?.status === 'success' && Number(v?.data?.amount ?? 0) >= Number(row.fee) * 100) {
    ...
    await admin.from('notifications').insert({ user_id: row.user_id, title: 'CAC application received ✅', body: reference + ' is now in our processing queue.' });
