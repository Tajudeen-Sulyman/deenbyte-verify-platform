...
export async function POST(req: NextRequest) {
  ...
  const st = await fetch('https://aijalon.ng/api/v1/val/status', { method: 'POST', headers: { Authorization: 'Bearer ' + process.env.AIJALON_TOKEN, 'Content-Type': 'application/json' }, body: JSON.stringify({ number: row.nin }) });
  const sj = await st.json().catch(() => ({}));
  if (String(sj.message ?? '').toLowerCase().includes('complet')) {
    const sl = await fetch('https://aijalon.ng/api/v1/val/slip', { method: 'POST', headers: { Authorization: 'Bearer ' + process.env.AIJALON_TOKEN, 'Content-Type': 'application/json' }, body: JSON.stringify({ number: row.nin, type: row.slip_type === 'prem' ? 'prem' : 'nonprem' }) });
