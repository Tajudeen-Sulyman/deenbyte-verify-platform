...
export async function GET(req: NextRequest) {
  ...
  if (vj.status && vj.data.status === 'success') {
    ...
    if (row && row.status === 'awaiting_payment') {
      ...
      const sub = await fetch('https://aijalon.ng/api/v1/val', { method: 'POST', headers: { Authorization: 'Bearer ' + AIJ, 'Content-Type': 'application/json' }, body: JSON.stringify({ number: row.nin }) });
