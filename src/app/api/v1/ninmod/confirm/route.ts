...
export async function GET(req: NextRequest) {
  ...
  const { data: row } = await admin.from('nin_mod_requests').select('*').eq('reference', reference).maybeSingle();
  ...
  if (v?.status === true && v?.data?.status === 'success' && Number(v?.data?.amount ?? 0) >= Number(row.fee) * 100) {
    ...
    return NextResponse.json({ confirmed: true, status: 'failed', error: sub.error });
  }
