      ...
      <div className="w-full max-w-md rounded-2xl border border-border bg-white p-6 text-center">
        ...
        {status && status !== 'awaiting_payment' && (
          <>
            ...
            <h1 className="mt-4 text-xl font-extrabold text-emerald-950">Application Received!</h1>
            ...
            <div className="mt-4 rounded-xl border-2 border-dashed border-border p-3">
              ...
              <button onClick={() => { navigator.clipboard.writeText(ref); setCopied(true); setTimeout(() => setCopied(false), 1500); }} className="mt-2 rounded-lg bg-emerald-800 text-white text-xs font-bold px-3 py-2">{copied ? 'Copied' : 'Copy'}</button>
            </div>
        ...
        {status === 'awaiting_payment' && (<><h1 className="text-lg font-extrabold text-emerald-950">Payment not confirmed yet</h1><p className="mt-2 text-xs text-muted">If you paid, refresh in a minute. Reference: <b className="text-dark">{ref}</b></p></>)}
      </div>
    </div>
  );
}
