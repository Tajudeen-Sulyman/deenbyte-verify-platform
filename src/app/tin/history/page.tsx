    ...
    <div className="min-h-screen bg-light pb-16">
      <header className="bg-emerald-950 text-white px-4 py-8">
        <div className="max-w-2xl mx-auto">
          <Link href="/tin" className="text-xs font-bold text-emerald-300">← New application</Link>
          <h1 className="mt-1 text-2xl font-extrabold">Tax ID (TIN) History</h1>
          <p className="mt-1 text-sm text-emerald-100">Track your requests and copy your generated Tax IDs.</p>
        </div>
      </header>
            ...
            <div key={r.reference} className="rounded-2xl border border-border bg-white p-4">
              ...
              {r.status === 'completed' && r.issued_tin && (
                <div className="mt-3 flex items-center justify-between rounded-xl bg-green-50 border border-green-200 px-3 py-2">
                  ...
                  <button onClick={() => navigator.clipboard.writeText(r.issued_tin)} className="text-xs font-bold text-emerald-800 underline">Copy ID</button>
                </div>
              )}
