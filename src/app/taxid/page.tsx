...
export default function TaxIdPage() {
  ...
  return (
    <div className="min-h-screen bg-light pb-16">
      <header className="bg-violet-950 text-white text-center px-4 py-12">
        <div className="mx-auto h-16 w-16 rounded-full border-4 border-violet-400 flex items-center justify-center">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="h-8 w-8 text-violet-300"><path d="M5 13l4 4L19 7" /></svg>
        </div>
        <p className="mt-4 text-xs font-bold tracking-[0.25em] text-violet-300">DEENBYTE VERIFY</p>
        <h1 className="mt-2 text-2xl sm:text-3xl font-extrabold">Get Your TIN Verification Slip</h1>
        <p className="mt-3 text-sm text-violet-100 max-w-md mx-auto">No account. No wallet funding. Pay once, and your slip is ready in under 90 seconds.</p>
        <span className="inline-flex items-center gap-2 mt-5 rounded-full bg-white/10 px-4 py-2 text-xs font-semibold"><span className="h-2 w-2 rounded-full bg-violet-400" />Verified &amp; instant</span>
      </header>

      <main className="max-w-2xl mx-auto p-4 space-y-5 mt-5">
        <section className="rounded-2xl border border-border bg-white p-5">
          <h2 className="flex items-center gap-3 text-sm font-extrabold tracking-widest text-violet-950"><span className="h-8 w-8 rounded-full bg-violet-950 text-white text-sm font-bold flex items-center justify-center">1</span>CHOOSE YOUR SLIP</h2>
          <div className="grid grid-cols-2 gap-3 mt-4">
            <button onClick={() => setTier('standard')} className={'relative rounded-xl border-2 p-4 text-center ' + (tier === 'standard' ? 'border-violet-700 bg-violet-50' : 'border-border bg-white')}>
              <p className="text-base font-extrabold text-dark">STANDARD</p>
              <p className="text-xs text-muted mt-1">Validation summary slip</p>
              <p className="text-lg font-extrabold text-violet-800 mt-2">₦50</p>
            </button>
            <button onClick={() => setTier('premium')} className={'relative rounded-xl border-2 p-4 text-center ' + (tier === 'premium' ? 'border-violet-700 bg-violet-50' : 'border-border bg-white')}>
        ...
        <section className="rounded-2xl border border-border bg-white p-5">
          <h2 className="flex items-center gap-3 text-sm font-extrabold tracking-widest text-violet-950"><span className="h-8 w-8 rounded-full bg-violet-950 text-white text-sm font-bold flex items-center justify-center">2</span>SELECT SLIP TYPE</h2>
          <div className="grid grid-cols-2 gap-3 mt-4">
            <button onClick={() => setSlipType('corporate')} className={'rounded-xl border-2 p-4 text-center ' + (slipType === 'corporate' ? 'border-violet-700 bg-violet-50' : 'border-border bg-white')}>
              <p className="text-2xl">🏢</p>
              <p className="font-bold text-dark mt-1">Corporate</p>
              <p className="text-xs text-muted">For registered businesses</p>
            </button>
            <button onClick={() => setSlipType('individual')} className={'rounded-xl border-2 p-4 text-center ' + (slipType === 'individual' ? 'border-violet-700 bg-violet-50' : 'border-border bg-white')}>
              <p className="text-2xl">👤</p>
        ...
        <section className="rounded-2xl border border-border bg-white p-5">
          <h2 className="flex items-center gap-3 text-sm font-extrabold tracking-widest text-violet-950"><span className="h-8 w-8 rounded-full bg-violet-950 text-white text-sm font-bold flex items-center justify-center">3</span>ENTER YOUR DETAILS</h2>
        ...
        <section className="rounded-2xl border border-border bg-white p-5">
          <h2 className="flex items-center gap-3 text-sm font-extrabold tracking-widest text-violet-950"><span className="h-8 w-8 rounded-full bg-violet-950 text-white text-sm font-bold flex items-center justify-center">4</span>DELIVERY DETAILS</h2>
        ...
        <section className="rounded-2xl border border-border bg-white p-5">
          <h2 className="flex items-center gap-3 text-sm font-extrabold tracking-widest text-violet-950"><span className="h-8 w-8 rounded-full bg-violet-950 text-white text-sm font-bold flex items-center justify-center">5</span>PAYMENT</h2>
          <div className="mt-4 rounded-xl bg-violet-950 text-white px-4 py-4 flex items-center justify-between">
        ...
        <section className="rounded-2xl border-2 border-dashed border-violet-300 bg-violet-50 p-5">
          <h3 className="text-sm font-extrabold text-violet-950">⚠️ ALREADY PAID BUT DIDN&apos;T GET YOUR SLIP?</h3>
          ...
          <a href={rec.trim() ? '/taxid/success?reference=' + encodeURIComponent(rec.trim()) : '#'} className="mt-3 block w-full rounded-xl border-2 border-violet-900 text-violet-950 font-extrabold py-3 text-center text-sm">VERIFY &amp; DOWNLOAD MY SLIP</a>
        </section>
      </main>
    </div>
  );
